...
fun <- function(outfile) {
    outfile <- paste(outfile, "_phyml_stats.txt", sep = "")
    l <- readLines(outfile)
    ...
    names(loglik) <-  c("JC69", "JC69+I", "JC69+G", "JC69+I+G",
      "K80", "K80+I", "K80+G", "K80+I+G",
      "F81", "F81+I", "F81+G", "F81+I+G",
      "F84", "F84+I", "F84+G", "F84+I+G",
      "HKY85", "HKY85+I", "HKY85+G", "HKY85+I+G",
      "TN93", "TN93+I", "TN93+G", "TN93+I+G",
      "GTR", "GTR+I", "GTR+G", "GTR+I+G")[imod]
                      ...
                      exclude = NULL, execname = NULL, append = TRUE)
{
    ...
    outfile <- paste(seqfile, "_phyml_stats", sep = "")
    if (!append) {
        unlink(outfile)
        unlink(paste(seqfile, "_phyml_tree", sep = ""))
    }
    imod <- 1:N
